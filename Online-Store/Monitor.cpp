...
void Monitor::input()
{
	...
	cout << "Enter price : ";
	cin >> priceMonitor;
	ofstream fout("Products\\Monitor.csv", ios::out | ios::app);
	if (fout.is_open())
	{
		fout << "-------" << name << "-------" << endl;
		fout << name << "   " << '\"' << name << '\"' << endl;
...
void Monitor::deleteProducts()
{
	...
	cin.ignore(32767, '\n');
	getline(cin, name);
	ifstream fout("Products\\Monitor.csv");
	if (fout.is_open())
	{
	...
	fout.close();
	ofstream fsut("Products\\Monitor.csv", std::ofstream::out | std::ofstream::trunc);
	fsut.close();
	ofstream fomt("Products\\Monitor.csv", ios::out | ios::app);
	if (fomt.is_open())
	{
